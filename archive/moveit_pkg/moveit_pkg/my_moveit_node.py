...
class MoveItPoseGoalNode(Node):
    def __init__(self):
        ...
        try:
            self.controllers = list(self.get_parameter("controllers").get_parameter_value().string_array_value)
        except Exception:
            self.controllers = ["joint_trajectory_controller"]
            
        # Create MoveItPy context and planning component
        print(f"Planning group: {self.get_name()}")
    ...
    def _plan_and_execute(self):
        ...
        exec_status = self.moveit.execute(last_plan, controllers=self.controllers)
        # If no exception thrown, consider it success. Log status object for visibility
        self.get_logger().info(f"Execution returned: {exec_status}")
