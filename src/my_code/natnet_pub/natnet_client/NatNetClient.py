...
import my_code.natnet_pub.natnet_client.DataDescriptions as DataDescriptions
import my_code.natnet_pub.natnet_client.MoCapData as MoCapData
...
class NatNetClient:
    ...
    def new_frame_with_data_listener(self, frame_data):
        """This function is called when a new frame of data is received.
        It can be overridden by the user to handle the data as needed."""
    ...
    def __command_thread_function(self, in_socket, stop, gprint_level, thread_option):  # type: ignore  # noqa E501
        ...
        while not stop():
            ...
            try:
                ...
                buffer_list[buffer_list_recv_index][: len(data)] = data
                ...
                if stop():
                    print("shutting down with error:", msg)
            ...
            # else:
            #     # No data received, check if we should send a keep alive
            #     if self.use_multicast:
    ...
    def send_request(self, in_socket, command, command_str, address):
        ...
        if command == self.NAT_CONNECT:
            ...
            if isinstance(command_str, str):
                ...
                # command_str is a byte array
                print("command_str is not a string, but a byte array")
        data += b"\0"
    ...
    def run(self, thread_option):
        ...
        # Example Commands
        # Get NatNet and server versions
        # server_ver = self.send_request(self.command_socket, self.NAT_CONNECT, "", (self.server_ip_address, self.command_port)) #type: ignore  # noqa E501
        # # Request the model definitions
        # model_def = self.send_request(self.command_socket, self.NAT_REQUEST_MODELDEF, "", (self.server_ip_address, self.command_port)) #type: ignore  # noqa E501
    ...
    def shutdown(self):
        ...
        if self.command_thread is not None:
            print("joining command thread")
            if self.command_thread.is_alive():
                self.command_thread.join()
        if self.data_thread is not None:
            print("joining data thread")
            if self.data_thread.is_alive():
                self.data_thread.join()

    ## my new methods
    def extract_mocap_data_to_dict(self):
        """
        Extract all rigid bodies, their markers, and unlabeled markers into a structured dictionary.

        Returns:
            dict: Structured data containing rigid bodies and markers
        """
        if (
            self.current_frame_data is None
            or "mocap_data" not in self.current_frame_data
        ):
            return None

        mocap_data: MoCapData.MoCapData = self.current_frame_data["mocap_data"]
        result_dict = {
            "frame_number": self.current_frame_data.get("frame_number", 0),
            "timestamp": self.current_frame_data.get("timestamp", 0.0),
            "rigid_bodies": {},
            "labeled_markers": {},
            "unlabeled_markers": {},
        }
        ...
        rigid_body_data = mocap_data.get_rigid_body_data()
        if rigid_body_data is not None:
            rigid_bodies = rigid_body_data.get_rigid_body_list()
            for i, rb in enumerate(rigid_bodies):
                rb_dict = {
                    ...
                    "tracking_valid": getattr(rb, "tracking_valid", True),
                    "error": getattr(rb, "error", 0.0),
                    "markers": {},
                }

                # Extract markers for this rigid body
                rb_markers = rb.get_rigid_body_marker_list()
                if rb_markers:
                    for j, marker in enumerate(rb_markers):
                        rb_dict["markers"][f"marker_{j}"] = {
                            "id": getattr(marker, "id", -1),
                            "position": [
                                marker.pos[0],
                                marker.pos[1],
                                marker.pos[2],
                            ],
                            "size": getattr(marker, "size", 0.0),
                        }

                result_dict["rigid_bodies"][f'rigid_body_{rb_dict["id"]}'] = rb_dict
        else:
            print("No rigid body data available in the current frame.")
        # Extract Labeled Markers
        labeled_marker_data = mocap_data.get_labeled_marker_data()
        if labeled_marker_data:
            labeled_markers = labeled_marker_data.get_labeled_marker_list()
            for i, marker in enumerate(labeled_markers):
                # Decode marker ID to get model_id and marker_id
                model_id, marker_id = self.__decode_marker_id(marker.id_num)

                if model_id == 0:
                    model_key = "unlabeled"
                else:
                    model_key = f"stream_id_{model_id}"
                if model_key not in result_dict["labeled_markers"]:
                    result_dict["labeled_markers"][model_key] = []

                result_dict["labeled_markers"][model_key].append(
                    {
                        "marker_id": marker_id,
                        "position": [
                            marker.pos[0],
                        ...
                        "size": marker.size,
                        "residual": getattr(marker, "residual", 0.0),
                        "occluded": (
                            (marker.param & 0x01) != 0
                            if hasattr(marker, "param")
                            else False
                        ),
                        "point_cloud_solved": (
                            (marker.param & 0x02) != 0
                ...
                # result_dict['labeled_markers'][f'marker_{i}'] = {
                #     'id': marker.id_num,
                #     'model_id': model_id,
                #     'marker_id': marker_id,
                #     'position': {
                ...
                #     'size': marker.size,
                #     'residual': getattr(marker, 'residual', 0.0),
                #     'occluded': (marker.param & 0x01) != 0 if hasattr(marker, 'param') else False,
                #     'point_cloud_solved': (marker.param & 0x02) != 0 if hasattr(marker, 'param') else False,
                #     'model_solved': (marker.param & 0x04) != 0 if hasattr(marker, 'param') else False
                # }

        # Extract Unlabeled Markers (from marker set data)
        # marker_set_data = mocap_data.get_marker_set_data()
        # if marker_set_data:
        #     unlabeled_markers = marker_set_data.get_unlabeled_marker_data()
        #     for i, marker_pos in enumerate(unlabeled_markers.get_marker_pos_list()):
        #         result_dict['unlabeled_markers'][f'marker_{i}'] = {
        #             'position': {
        #                 'x': marker_pos[0],
        #                 'y': marker_pos[1],
        #                 'z': marker_pos[2]
    ...
    def get_structured_mocap_data(self):
        """
        Convenience method to get structured mocap data.
        Returns None if no data is available.
        """
        return self.extract_mocap_data_to_dict()
...
if __name__ == "__main__":
    ...
    try:
        ...
        # This is a test run of the NatNetClient
        # It will not work without a server running
        print("This is a test run of the NatNetClient")
        print("It will not work without a server running")

        client = NatNetClient()
        client.set_server_address("132.66.210.173")
        client.set_client_address("132.66.53.99")
        
        client.set_use_multicast(False)
        # 
        client.set_print_level(0)  # Set print level to 1 for more detailed output
        # Streaming client configuration.

        # client.rigid_body_listener = receive_rigid_body_frame
        is_running = client.run(thread_option="d")
        if not is_running:
            ...
            try:
                ...
                pass
        time.sleep(1)
        # client.set_nat_net_version(4, 3)
        # ret = client.send_request(client.command_socket, client.NAT_REQUEST_MODELDEF, "", (client.get_server_address(), client.get_command_port()))
        # print("Requesting model definitions, return code:", ret)  
        if not client.connected():
            ...
            for i in range(3):
                ...
                if connected:
                    print("Connected successfully after retrying.")
                    break
                time.sleep(3)
                print(f"Retrying connection... ({i + 1}/3)")
            if not connected:
                print("Failed to connect after retries. Exiting.")
        ...
        mocap_data = client.get_structured_mocap_data()
        if mocap_data:
            pprint(mocap_data)
        else:
            print("No mocap data available.")
